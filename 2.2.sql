CREATE OR REPLACE PROCEDURE dm.update_loan_holiday_info()
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
	...
	DROP TABLE IF EXISTS dm.temp_holiday_info;
	CREATE TEMP TABLE temp_holiday_info AS
		WITH deal AS (
			SELECT  deal_rk
		...
		SELECT DISTINCT hi.deal_rk
	      ,hi.effective_from_date
	      ,hi.effective_to_date
	      ,hi.agreement_rk
	      ,hi.client_rk
	      ,hi.department_rk
	      ,hi.product_rk
	      ...
	      ,hi.deal_number
	      ,hi.deal_sum
	      ,hi.loan_holiday_type_cd
	      ,hi.loan_holiday_start_date
	      ,hi.loan_holiday_finish_date
	      ,hi.loan_holiday_fact_finish_date
	      ,hi.loan_holiday_finish_flg
	      ,hi.loan_holiday_last_possible_date
		FROM holiday_info AS hi
		LEFT JOIN dm.loan_holiday_info AS lhi
			ON hi.deal_rk = lhi.deal_rk
			AND hi.effective_from_date = lhi.effective_from_date
			AND hi.deal_number = lhi.deal_number
		WHERE lhi.deal_rk IS NULL;

	rows_affected := (
		SELECT COUNT(*)
		FROM temp_holiday_info
	);
	...
	DROP TABLE temp_holiday_info;

	end_time := clock_timestamp();

	CALL logs.proc_logs(
		'dm.update_loan_holiday_info'
		, start_time
		, end_time
		, rows_affected
	);
